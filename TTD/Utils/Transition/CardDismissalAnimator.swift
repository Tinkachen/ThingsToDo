...
final class DismissCardAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let cardDetail: CardContentDetailViewController = (ctx.viewController(forKey: .from) as? UINavigationController)?.viewControllers.first as? CardContentDetailViewController,
        ...
        // Fix weird top inset
        let topTemporaryFix = cardDetail.cardContentView.topAnchor.constraint(equalTo: cardDetailView.topAnchor)
        topTemporaryFix.isActive = TransitionConstants.isEnabledWeirdTopInsetsFix
        
        container.layoutIfNeeded()
        
        // Force card filling bottom
        let stretchCardToFillBottom = cardDetail.cardContentView.bottomAnchor.constraint(equalTo: cardDetailView.bottomAnchor)
        
        /// Animates the card view back to place
